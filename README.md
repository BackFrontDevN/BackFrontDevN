# Nurullo Musajanov
<div align="center">

![Typing Banner](https://readme-typing-svg.demolab.com?font=Fira%20Code&weight=700&size=36&duration=3000&pause=1000&color=ffffff&center=true&width=900&height=80&lines=Python+Backend+Developer;Django+Developer;REST+API+Enthusiast)

</div>

---

## About me
<div align="center">

Backend developer working with Python and Django. I focus on building clear, maintainable APIs and backend components. Open to remote Python/Django opportunities. Based in Osh, Kyrgyzstan.

</div>

---

## Tech Stack
<div align="center">

![Tech stack](https://skillicons.dev/icons?i=python,django,postgres,docker,fastapi,js,html,css,git&theme=dark)

</div>

---

## Current focus
<div align="center">

• Django · Django REST Framework · PostgreSQL · Docker (learning) · FastAPI (learning)

</div>

---

## Featured project — Nerzhaveyka
<div align="center">

**Developed independently.** A Django application with a backend-first structure—models, views, and API endpoints are implemented in repository code. See the repo for exact details and instructions.

**Technologies:** Python · Django · Django REST Framework · PostgreSQL / SQLite · HTML/CSS

<br/>

<a href="https://github.com/nm-backend/nerzhaveyka"><img src="https://img.shields.io/badge/Repository-Nerzhaveyka-111827?style=for-the-badge&logo=github&logoColor=white" alt="Repository — Nerzhaveyka" /></a>

</div>

<!--
  Screenshot placeholder (hidden until a file exists).

  To display a screenshot:
   1) Add the image at `assets/nerzhaveyka_screenshot.png` in this repository.
   2) Remove this comment block and place the image with a simple centered tag:

   <div align="center">
     <img src="assets/nerzhaveyka_screenshot.png" alt="Nerzhaveyka screenshot" />
   </div>

  Keep images focused on the UI or API console; avoid personal data in screenshots.
-->

---

## GitHub analytics
<div align="center">

<!-- Core stats -->
<img src="https://github-readme-stats.vercel.app/api?username=nm-backend&show_icons=true&theme=dark&hide_border=true" alt="GitHub readme stats" />

<!-- Top languages -->
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=nm-backend&layout=compact&theme=dark&hide_border=true" alt="Top languages" />

<!-- Profile summary cards -->
<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=nm-backend&theme=dark" alt="Profile summary" />

<!-- Trophies -->
<img src="https://github-profile-trophy.vercel.app/?username=nm-backend&theme=dark" alt="Trophies" />

<!-- Contribution graph -->
<img src="https://ghchart.rshah.org/nm-backend" alt="Contribution graph" />

<!-- Streak & activity graph -->
<img src="https://github-readme-streak-stats.herokuapp.com/?user=nm-backend&theme=dark&hide_border=true" alt="GitHub streak stats" />
<img src="https://github-readme-activity-graph.vercel.app/graph?username=nm-backend&theme=react-dark" alt="Activity graph" />

</div>

---

## Connect with me
<div align="center">

<a href="https://github.com/nm-backend" title="GitHub"><img src="https://img.shields.io/badge/GitHub-nm--backend-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub — nm-backend" /></a>
&nbsp;
<a href="https://www.linkedin.com/in/nurullo-musajanov-040b31337/" title="LinkedIn"><img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
&nbsp;
<a href="https://t.me/nm_2110" title="Telegram"><img src="https://img.shields.io/badge/Telegram-Message-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white" alt="Telegram" /></a>

</div>

---

<div align="center">

Design: modern · minimal · premium · dark-friendly.  
Responsive and recruiter-focused — concise sections and clear links to code.

</div>
